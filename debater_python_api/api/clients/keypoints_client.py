    ...
    def write_result_to_csv(result, result_file):
        ...
        * summary file: a summary file with all key points and their aggregated information, saved in result_file path with suffix: _kps_summary.csv.
        ...
        for keypoint_matching in result['keypoint_matchings']:
            ...
            summary_rows.append([kp, n_sentences, sentence_coverage, n_comments, comments_coverage])
        ...
        summary_df = pd.DataFrame(summary_rows, columns=["kp", "#sentences", 'sentences_coverage', '#comments', 'comments_coverage'])
        ...
        if len(set(kp_to_parent.values())) > 1:
            ...
            KpAnalysisUtils._write_df_to_file(hierarchy_df, hierarchy_file)

        summary_file = result_file.replace(".csv", "_kps_summary.csv")
        KpAnalysisUtils._write_df_to_file(summary_df, summary_file)
        ...
        KpAnalysisUtils._write_df_to_file(match_df, result_file)
...
class KpAnalysisClient(AbstractClient):
    ...
    def get_sentneces_for_domain(self, domain: str, job_id: Optional[str] = None):
        '''
